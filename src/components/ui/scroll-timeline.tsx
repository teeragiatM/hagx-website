...
export function ScrollTimeline({
  ...
  return (
    ...
    >
      <section
        ref={sectionRef}
        className={cn('ui-scroll-timeline relative overflow-hidden', className)}
        {...props}
      >
...
export function ScrollTimelineTrack({
  ...
  return (
    <div className={cn('ui-scroll-timeline-track relative', className)}>
      <div className="absolute top-0 left-5 h-full w-px bg-white/[0.08] lg:left-1/2" />
      <motion.div
        style={{ scaleY: scrollYProgress }}
        className="absolute top-0 left-5 h-full w-px origin-top bg-[#E15F31]/70 lg:left-1/2"
      />
  ...
  return (
    ...
    >
      <motion.div
        ...
        className="absolute top-2 left-5 z-20 flex h-6 w-6 -translate-x-1/2 items-center justify-center rounded-full border border-[#E15F31]/60 text-xs font-medium text-accent-500 shadow-[0_0_18px_rgba(255,138,0,0.3)] lg:left-1/2 lg:-translate-x-1/2"
      >
        {item.n}
      </motion.div>
      ...
      >
        ...
        >
          {item.year && (
            <p
              className={cn(
                'mb-1 text-xs font-bold tracking-tight transition-colors duration-500',
                isActive ? 'text-accent-500' : 'text-foreground-400'
              )}
            >
              {item.year}
            </p>
          ...
          <p
            className={cn(
              'mb-3 text-xs font-light tracking-widest uppercase transition-colors duration-500',
              isActive ? 'text-accent-500' : 'text-foreground-400'
            )}
          >
            {item.phase}
          ...
          <h3
            className={cn(
              'max-w-lg text-2xl leading-tight font-light transition-colors duration-500 sm:text-3xl',
              isActive ? 'text-foreground-100' : 'text-foreground-400'
            )}
          >
            {item.title}
          </h3>
          <div className="mt-5 space-y-3">
            {item.body.map((text) => (
              <p
                key={text}
                className={cn(
                  'max-w-lg text-xs leading-7 font-light transition-colors duration-500',
                  isActive ? 'text-foreground-300' : 'text-foreground-200'
                )}
              >
                {text}
              </p>
