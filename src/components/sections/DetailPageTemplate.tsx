      ...
      {coverImage && (
        <div className="relative h-[55vh] overflow-hidden">
          ...
          <div className="absolute right-0 bottom-0 left-0 px-8 pb-12 sm:px-14">
            {eyebrow && (
              <p className="mb-3 text-[10px] font-light tracking-widest text-accent-500 uppercase">
                {eyebrow}
              </p>
            )}
            ...
            {subtitle && (
              <p className="mt-3 text-sm font-light text-foreground-300">
                {subtitle}
              </p>
            )}
          </div>
      ...
      <section className="px-8 py-16 sm:px-14">
        <div className="grid gap-16 lg:grid-cols-3">
          ...
          <div className="space-y-10 lg:col-span-2">
            ...
            {body && (
              <div className="text-sm leading-8 font-light text-foreground-300">
                {body}
              </div>
            )}
            ...
            {highlights && highlights.length > 0 && (
              <div>
                <p className="mb-6 text-[10px] font-light tracking-widest text-foreground-400 uppercase">
                  Highlights
                </p>
                <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                  {highlights.map((h) => (
                    <div
                      key={h}
                      className="border border-white/[0.07] bg-background-200 p-4"
                    >
                      <span className="mb-1 block h-0.5 w-6 bg-accent-500" />
                      <p className="text-xs leading-5 font-light text-foreground-200">
                        {h}
                      </p>
                    </div>
                  ))}
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {related.map((r) => (
              <Link
                key={r.href}
                href={r.href}
                className="group block border border-white/[0.07] bg-background-200 transition-colors hover:border-accent-500/30"
              >
