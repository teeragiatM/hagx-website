...
const categoryColors: Record<string, string> = {
  TECHNICAL: "text-[#DB5828] border-[#DB5828]/40 bg-[#DB5828]/[0.06]",
...
function renderBody(md: string) {
  return md.split("\n\n").map((block, i) => {
    ...
    return (
      <p
        key={i}
        className="mb-5 text-sm font-light leading-[1.9] text-white/60"
      >
        {parts}
      </p>
    );
  ...
  return (
    <main className="min-h-screen bg-[#0a0a0a] text-white">
      ...
      <motion.div
        ...
        className="fixed left-0 top-0 z-[100] h-[2px] w-full bg-[#DB5828] shadow-[0_0_12px_rgba(255,138,0,0.7)]"
      />
        ...
        <div className="relative flex h-full flex-col justify-end px-6 pb-14 sm:px-10 lg:px-16">
          ...
          >
            ...
            <div className="mb-5 flex items-center gap-2 text-[10px] font-light uppercase tracking-widest text-white/30">
              ...
              <span>/</span>
              <span
                className={`border px-2 py-0.5 ${categoryColors[post.category] ?? "text-white/40 border-white/15"}`}
              >
                {category}
              </span>
            </div>
            ...
            <div className="mt-5 flex flex-wrap items-center gap-5 text-[11px] font-light text-white/35">
              ...
              <span>
                {post.readMin} {lang === "th" ? "นาทีในการอ่าน" : "min read"}
              </span>
            </div>
          </motion.div>
        </div>
      ...
      <div className="mx-auto grid max-w-[1500px] gap-0 px-6 pb-8 pt-14 sm:px-10 lg:grid-cols-[1fr_300px] lg:gap-16 lg:px-14">
        ...
        <article ref={articleRef}>
          {/* lead */}
          <p className="mb-10 border-l-2 border-[#DB5828] pl-6 text-base font-light leading-[1.9] text-white/65 sm:text-lg">
            {excerpt}
          </p>

          <div className="prose-invert max-w-none">{renderBody(body)}</div>

          {/* divider */}
          <div className="my-14 h-px bg-gradient-to-r from-[#DB5828]/30 via-white/10 to-transparent" />

          {/* ── CTA ── */}
          <div className="border border-[#DB5828]/20 bg-[#DB5828]/[0.04] p-8 sm:p-10">
            <p className="mb-2 text-[10px] font-light uppercase tracking-widest text-[#DB5828]">
            ...
            <Link
              href="/contact"
              className="inline-flex items-center gap-3 bg-[#DB5828] px-8 py-3.5 text-[11px] font-semibold uppercase tracking-widest text-black transition-all duration-300 hover:bg-[#ff9f33]"
            >
            ...
            <div className="sticky top-28">
              <div className="border border-white/[0.08] bg-[#121212]/80 backdrop-blur-sm">
                <div className="border-b border-white/[0.06] px-6 py-4">
                  <p className="text-[10px] font-light uppercase tracking-[0.18em] text-[#DB5828]">
                ...
                <div className="divide-y divide-white/[0.05] px-6">
                  {post.specs.map((s) => (
                    <div key={s.label} className="py-4">
                      <p className="mb-1 text-[9px] font-light uppercase tracking-widest text-white/30">
                        {s.label}
                      </p>
          ...
          <div className="mx-auto max-w-[1500px]">
            ...
            <div className="grid gap-6 sm:grid-cols-2">
              {related.map((rp) => (
                ...
                >
                  <div className="flex gap-5 border border-white/[0.07] bg-[#121212]/80 p-5 transition-all duration-300 hover:border-[#DB5828]/30 hover:shadow-[0_0_24px_rgba(255,138,0,0.08)]">
                    <div className="relative h-20 w-28 shrink-0 overflow-hidden">
                      <Image
                        src={rp.image}
                        alt={lang === "th" ? rp.titleTh : rp.title}
                        fill
                        sizes="112px"
                    ...
                    <div className="min-w-0">
                      <span
                        className={`mb-2 inline-block border px-2 py-0.5 text-[9px] font-light uppercase tracking-widest ${categoryColors[rp.category] ?? "text-white/40 border-white/15"}`}
                      >
