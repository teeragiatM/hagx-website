        ...
        <div className="mx-auto max-w-[1500px]">
          <div className="grid gap-12 lg:grid-cols-2 lg:gap-20">
              ...
              {p.images.length > 1 && (
                <div className="grid grid-cols-3 gap-3">
                  {p.images.slice(1).map((img, i) => (
                    <div key={i} className="relative aspect-[4/3] overflow-hidden border border-white/[0.07] bg-[#0c0c0c]">
                      <Image src={img} alt={`${p.nameEn} ${i + 2}`} fill sizes="20vw" className="object-cover opacity-60" />
                    </div>
                  ))}
                </div>
              )}
            ...
            <div className="flex flex-col">
              ...
              <h1 className="mb-2 text-3xl font-bold leading-tight tracking-tight sm:text-4xl">{p.name}</h1>
              <p className="mb-6 text-sm font-light text-white/35">{p.nameEn}</p>

              <div className="mb-6 border-l-2 border-[#ff8a00] pl-4">
                <p className="text-sm font-light italic text-white/50">{p.tagline}</p>
              </div>

              <p className="mb-8 text-sm font-light leading-8 text-white/50">{p.desc}</p>
              ...
              <div className="mb-8 border border-white/[0.07] bg-[#0c0c0c] p-5">
                ...
                <p className="text-3xl font-bold text-[#ff8a00]">
                  ...
                  <span className="ml-2 text-sm font-light text-white/30">/{p.unit}</span>
                </p>
                <p className="mt-2 text-[11px] font-light text-white/25">* ราคานี้เป็นราคาเริ่มต้น ขึ้นอยู่กับขนาดและปริมาณ</p>
              </div>

              {/* Specs */}
              <div className="mb-8">
                <p className="mb-4 text-[10px] font-light uppercase tracking-widest text-white/25">ข้อมูลจำเพาะ</p>
                <div className="divide-y divide-white/[0.06] border border-white/[0.06]">
                  {p.specs.map((s) => (
                    <div key={s.label} className="flex items-center justify-between px-4 py-3">
                      <span className="text-xs font-light text-white/35">{s.label}</span>
                      <span className="text-xs font-light text-white/70">{s.value}</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="mt-auto flex flex-col gap-3 sm:flex-row">
                <Link href="/contact" className="btn btn-primary flex-1 text-center">
                  สอบถามราคา / สั่งซื้อ
                </Link>
      ...
      {related.length > 0 && (
        <section className="border-t border-white/[0.06] px-8 py-16 sm:px-14">
          <div className="mx-auto max-w-[1500px]">
            ...
            <h2 className="mb-10 text-2xl font-bold tracking-tight">Related Products</h2>
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {related.map((r) => (
                ...
                >
                  ...
                  <div className="p-5">
                    <p className="mb-1 text-[9px] font-light uppercase tracking-widest text-white/25">{r.subcategory}</p>
                    <h3 className="mb-1 text-sm font-semibold text-white">{r.name}</h3>
                    <p className="text-base font-semibold text-[#ff8a00]">
                      ฿{Number(r.priceFrom).toLocaleString()}
                      <span className="ml-1 text-[10px] font-light text-white/30">/{r.unit}</span>
                    </p>
                  </div>
                </Link>
              ))}
